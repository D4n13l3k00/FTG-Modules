...
class FDQuoteMod(loader.Module):
    ...
    async def fdqcmd(self, m: Message):
        ...
        name = telethon.utils.get_display_name(user) if type(user) == Channel else (
            self.strings['deleted_acc'] if user and user.deleted else telethon.utils.get_display_name(user))
        id = user.id
        ...
        if reply and catch_reply:
            ...
            if reply.photo:
                ...
                replText = self.strings['quiz'] if reply.media.poll.quiz else self.strings['poll']
            elif reply.sticker:
            ...
            reply = {
                "name": telethon.utils.get_display_name(r) if type(r) == Channel else (self.strings['deleted_acc'] if r and r.deleted else telethon.utils.get_display_name(r)),
                "text": replText
            }
        else:
            reply = None
        await utils.answer(m, self.strings['processing_api'])
        js = {"avatar": base64.b64encode(avatar).decode() if avatar else None,
              "name": name,
              "text": text,
              "id": id,
